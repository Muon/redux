...
def test_valid_parses():
    valid_parses = [
        ...
        ("for a = 1, a<100, a = a + 1 say(a) end", Block([ForStmt(Assignment(VarRef('a'), Constant(1, int_)), LessThanOp(VarRef('a'), Constant(100, int_)), Assignment(VarRef('a'), AddOp(VarRef('a'), Constant(1, int_))), Block([ExprStmt(FunctionCall('say', [VarRef('a')]))]))])),
        ...
        ("a = 2 > 0 and 1 > 0", Block([Assignment(VarRef('a'), LogicalAndOp(GreaterThanOp(Constant(2, int_), Constant(0, int_)), GreaterThanOp(Constant(1, int_), Constant(0, int_))), False)])),
        ...
        ("a = (QUERY UNIT WHERE query->HP > 100)", Block([Assignment(VarRef('a'), Query('UNIT', VarRef('unit'), 'MIN', Constant(1, int_), GreaterThanOp(ChronalAccess(VarRef('query'), 'HP'), Constant(100, int_))))])),
        ...
        ("a = (QUERY UNIT MIN query->HP)", Block([Assignment(VarRef('a'), Query('UNIT', VarRef('unit'), 'MIN', ChronalAccess(VarRef('query'), 'HP'), Constant(1, int_)))])),
        ("a = (QUERY UNIT MIN 1 WHERE query->HP)", Block([Assignment(VarRef('a'), Query('UNIT', VarRef('unit'), 'MIN', Constant(1, int_), ChronalAccess(VarRef('query'), 'HP')))])),
        ("a = QUERY VALUE SUM 1 WHERE unit->HP > 0 and 1 > 0", Block([Assignment(VarRef("a"), Query("VALUE", VarRef("unit"), "SUM", Constant(1, int_), LogicalAndOp(GreaterThanOp(ChronalAccess(VarRef("unit"), "HP"), Constant(0, int_)), GreaterThanOp(Constant(1, int_), Constant(0, int_)))))])),
        ("a = QUERY UNIT WHERE query->HP > 100", Block([Assignment(VarRef('a'), Query('UNIT', VarRef('unit'), 'MIN', Constant(1, int_), GreaterThanOp(ChronalAccess(VarRef('query'), 'HP'), Constant(100, int_))))])),
        ("a = QUERY BESTMOVE target MIN query->XPosition", Block([Assignment(VarRef("a"), Query("BESTMOVE", VarRef("target"), "MIN", ChronalAccess(VarRef("query"), "XPosition"), Constant(1, int_)))])),
        ("a = QUERY UNIT MIN query->HP", Block([Assignment(VarRef('a'), Query('UNIT', VarRef('unit'), 'MIN', ChronalAccess(VarRef('query'), 'HP'), Constant(1, int_)))])),
        ...
        ("a = QUERY BESTMOVE MIN 1 WHERE 1", Block([Assignment(VarRef('a'), Query('BESTMOVE', VarRef('unit'), 'MIN', Constant(1, int_), Constant(1, int_)))])),
        ...
        ("a = ~4", Block([Assignment(VarRef('a'), BitwiseNotOp(Constant(4, int_)))])),
        ("a = 1 % 4", Block([Assignment(VarRef('a'), ModuloOp(Constant(1, int_), Constant(4, int_)))])),
        ("a = -a", Block([Assignment(VarRef('a'), NegateOp(VarRef("a")))])),
        ("a = 2 ** 2", Block([Assignment(VarRef('a'), PowerOp(Constant(2, int_), Constant(2, int_)))])),
        ("require \"foo\"", Block([Require("foo")])),
    ]
